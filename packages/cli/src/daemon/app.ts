...
export interface GitHubWiring {
  ...
  /**
   * Fetches one PR's metadata by reference, for the add-PR route (POST /api/units). Separate from
   * `hydrate` on purpose: minting must not fetch a diff or spend a token on a narrative — the drill-in
   * hydrates lazily on open, exactly as it does for a polled unit. Throws on a bad ref (404) or API
   * failure; the route turns that into a message the field can render. Absent → the route 503s.
   */
...
export function createDaemonApp(deps: DaemonAppDeps): { app: Hono } {
  ...
  /**
   * Add any PR to the queue by reference — the command center's "review any PR" field. The other door
   * into the store (the poller mints what GitHub asks of you; this mints what you ask for).
   *
   * Body: `{ pr: "https://github.com/owner/repo/pull/123" | "owner/repo#123" }` — the same forms
   * `dad <pr>` takes, via the shared parser. A bare number is rejected here: the CLI resolves one
   * against the git remote in your cwd, and the daemon has no cwd to resolve against.
   *
   * Idempotent, and deliberately so *before* the network: re-adding a PR already in the queue (or one
   * the poller minted) returns that unit with `existing: true` so the field navigates you to the work
   * in progress instead of minting a second unit for one PR. Only genuinely new PRs cost a fetch.
   *
   * The minted unit is `pinned` (see ReviewUnit.pinned) — it isn't on your review-request list, so the
   * poller's reconciliation must not treat its absence there as a reason to delete it. Nothing is
   * fetched beyond the PR's metadata: the diff and the walkthrough are generated by the drill-in's
   * lazy hydrate on open, the same path a polled unit takes, so adding a PR is instant.
   */
  app.post('/api/units', async (c) => {
    ...
    const raw = (body.pr ?? '').trim();
    if (!raw) return c.json({ error: 'a PR reference is required' }, 400);
    ...
    if (!ref) {
      return c.json(
        {
          error: /^\d+$/.test(raw)
            ? `"${raw}" needs a repo — try owner/repo#${raw}`
            : `couldn't read "${raw}" as a PR — try https://github.com/owner/repo/pull/123 or owner/repo#123`,
        },
        400,
      );
    }
    ...
    if (!prFetcher) {
      return c.json(
        {
          error:
            'The daemon has no GitHub credentials — add a token on the Settings page, run `gh auth login`, or set DIFFDAD_GITHUB_TOKEN.',
        },
        503,
      );
    }
    ...
    try {
      metadata = await prFetcher(ref.owner, ref.repo, ref.number);
    } catch (err) {
      const message = err instanceof Error ? err.message : String(err);
      console.error(`[diffdad] add PR failed for ${repo}#${ref.number}: ${message}`);
      // The client throws the raw `GitHub API 404 ... : {...}` string; a wrong repo or a private one
      // you can't see are the two overwhelmingly likely causes, and both read the same from here.
      return c.json(
        { error: / 404 /.test(message) ? `${repo}#${ref.number} — no such PR, or you can't see it` : message },
        502,
      );
    }
    ...
    const unit = store.addGithubUnit({
      ...
      // GitHub's canonical PR permalink. Derived, not fetched: `PRMetadata` carries no html_url, and
      // this form is what every other surface links to.
      url: `https://github.com/${repo}/pull/${ref.number}`,
      baseRef: metadata.base,
      metadata,
      pinned: true,
    });
    broadcast('units', unitsPayload(store.list()));
