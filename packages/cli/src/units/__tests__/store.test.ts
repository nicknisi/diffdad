...
describe('UnitStore', () => {
  ...
  describe('refreshMetadata', () => {
    it('patches title/body/branch + counts, leaving status/headSha/reviewedSha/narrative untouched', async () => {
      const store = new UnitStore([], det());
      ...
      const after = store.refreshMetadata(u.unitId, {
        title: 'Add widgets v2',
      ...
      expect(after.metadata.title).toBe('Add widgets v2');
      expect(after.metadata.body).toBe('Fresh description.');
      expect(after.metadata.branch).toBe('feat/widgets');
      ...
      // A title heal rewrites taskLabel with it (the queue row's label).
      expect(after.taskLabel).toBe('Add widgets v2');
      // Everything the heal must NOT touch:
