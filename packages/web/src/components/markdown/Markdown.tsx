...
function renderInline(text: string): string {
  ...
  out = out.replace(
    /([\w-]+\/[\w-]+#\d+)/g,
    '<a href="#" style="color:var(--brand);text-decoration:underline">$1</a>',
  );
  return out;
}
...
const HTML_BLOCK_RE = /^<\/?(?:h[1-6]|p|div|details|summary|table|thead|tbody|tr|th|td|ul|ol|li|hr|br|pre|blockquote|section|article|aside|nav|header|footer|sub|sup|dl|dt|dd)[\s>/]/i;
...
function renderMarkdown(src: string, theme: "light" | "dark" = "light"): string {
  ...
  function flushPara() {
    ...
    if (!joined) { para = []; return; }
    out.push(
      `<p class="mb-3 text-base leading-relaxed" style="color:var(--fg-1)">${processInline(joined)}</p>`,
    );
    para = [];
  }
  ...
  while (i < lines.length) {
    ...
    if (line.includes("|") && line.trim().startsWith("|")) {
      ...
      while (i < lines.length) {
        ...
        const cells = cur.split("|").slice(1, -1).map((c) => c.trim());
        if (/^[-:|]+$/.test(cells.join(""))) {
          hasSeparator = true;
          i++;
          continue;
