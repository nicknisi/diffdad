...
(function () {
  ...
  function renderInline(src, keyPrefix = "i") {
    ...
    while (i < src.length) {
      ...
      if (c === "#" || (c.match(/[a-zA-Z0-9]/) && rest.match(ISSUE_RE) && /[\s(,.;!?]|^/.test(i === 0 ? " " : src[i - 1]))) {
      ...
      return (
        <div className="md-suggestion">
          <div className="md-suggestion-head">
            <span className="md-suggestion-icon">{(window.Icons && window.Icons.Spark) ? <window.Icons.Spark /> : null}</span>
            <span>Suggested change</span>
            <div className="md-suggestion-actions">
              <button className="md-suggestion-btn">Apply</button>
            </div>
          </div>
          <pre className="md-suggestion-body"><code>{content}</code></pre>
        </div>
      );
    }
    if (lang === "diff") {
          ...
          <code>
            {lines.map((ln, idx) => {
              const cls = ln.startsWith("+") && !ln.startsWith("+++") ? "diff-add" :
                          ln.startsWith("-") && !ln.startsWith("---") ? "diff-rem" :
                          ln.startsWith("@@") ? "diff-hunk" : "";
              return <span key={idx} className={cls}>{ln}{"\n"}</span>;
            })}
          </code>
        </pre>
      );
