...
function SuggestedStart({ onJump }) {
  ...
  return (
    <div className="suggest">
      <div className="ai-mark"><Icon.Spark /></div>
      <div className="body">
        <b>Suggested place to start:</b> Chapter 4 — <em>Wire discovery into the connection setup flow</em>. It's the user-visible change and has the highest blast radius. Yusuf already flagged a debounce concern there.
        <div className="actions">
          <button className="btn sm" onClick={() => onJump("ch4")}>Jump to chapter 4</button>
  ...
  return (
    <div className="narration-anchor">
      <div className="density">
        {[["terse", "Terse"], ["normal", "Normal"], ["verbose", "Verbose"]].map(([k, lab]) => (
          <button key={k} className={density === k ? "active" : ""} onClick={() => setDensity(k)}>{lab}</button>
        ))}
      </div>
      <button onClick={onRenarrate} disabled={renarrating}>
        <Icon.Refresh />{renarrating ? "Re-narrating…" : "Re-narrate"}
      </button>
      <button onClick={onAsk}><Icon.Spark />Ask AI</button>
      <button onClick={onCommentChapter}><Icon.Chat />Comment on chapter</button>
    </div>
  );
}
...
function ChapterCommentBlock({ thread }) {
  ...
  return (
    <div className="chap-thread">
      ...
      {thread.map(c => <window.DiffComponents.Comment key={c.id} c={c} />)}
    </div>
  );
}
...
function AiAsk({ open, onClose, hunkLabel }) {
  ...
  function ask() {
    ...
    setA("Loading…");
    setTimeout(() => {
      setA(`The 5-second timeout was chosen to match the existing <code>fetch()</code> wrapper's default. Microsoft Entra cold starts are typically 800ms-1.5s in the EU/US regions; 5s gives a comfortable margin. Adaeze's comment about cold starts is referencing the <em>tenant-suspended</em> path which can stall longer — that's worth handling separately rather than relaxing the global timeout.`);
    }, 600);
  }
  return (
    <div className="ai-ask">
      <div className="lab"><Icon.Spark />Ask AI about {hunkLabel || "this chapter"}</div>
      <div className="input">
        <textarea
        ...
        <button className="btn ghost" onClick={onClose}><Icon.X /></button>
      </div>
...
function Chapter({ ch, idx, density, setDensity, openLine, setOpenLine, addComment, reviewed, toggleReviewed, onJump, clusterBots, updatedHunks, structure = "chapters" }) {
  const Icon = window.Icons;
  ...
  const [chapThread, setChapThread] = React.useState(
    ch.id === "ch3" ? [{
      id: `chcom-${ch.id}`,
      author: "achen",
  ...
  return (
    <section className={`chapter ${reviewed ? "done" : ""} ${collapsed ? "collapsed" : ""}`} data-chid={ch.id}>
      <div className="chapter-head" onClick={structure === "outline" ? () => setCollapsed(v => !v) : undefined} style={structure === "outline" ? { cursor: "pointer" } : undefined}>
        ...
        <div className="titles">
          ...
          {structure === "outline" && (
            <div className="outline-meta">
              {ch.hunks.length} {ch.hunks.length === 1 ? "hunk" : "hunks"} · {ch.hunks.reduce((n, h) => n + (h.comments || []).length, 0)} comments
            </div>
          )}
        </div>
        {structure === "outline" && (
          <button className="outline-toggle" aria-label={collapsed ? "Open chapter" : "Collapse chapter"}>
            <Icon.ChevronDown style={{ transform: collapsed ? "rotate(-90deg)" : "rotate(0deg)", transition: "transform 120ms ease" }} />
          </button>
        )}
        <button className={`reviewed-toggle ${reviewed ? "on" : ""}`} onClick={(e) => { e.stopPropagation(); toggleReviewed(); }}>
          {reviewed ? <><Icon.Check />Reviewed</> : <>Mark reviewed</>}
        </button>
      </div>
        ...
        <>
          {renarrationLens && (
            <div className="renarrate-banner">
              <Icon.Spark />
              <span>Re-narrated for <b>{renarrationLens}</b> reviewers.</span>
              <button onClick={() => setRenarrationLens(null)}>Restore default</button>
            </div>
          )}
          <p className="narration" dangerouslySetInnerHTML={{ __html: renarrationLens ? renarrateNarration(Narrations[ch.id][density], renarrationLens) : Narrations[ch.id][density] }} />
          <NarrationAnchor
            density={density}
              ...
              <div className="thread" style={{ background: "transparent", boxShadow: "none", padding: 0 }}>
                <div className="reply-area">
                  ...
                  <div className="btnrow">
                    <button className="btn ghost sm" onClick={() => setChapComment(false)}>Cancel</button>
                    <button className="btn primary sm"><Icon.Send />Comment</button>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div key={file} className="filecard">
            <div className="head">
              ...
              <span className="right">
                <button className="btn surface sm"><Icon.Github />View on GitHub</button>
              </span>
            </div>
            {hunks.map((h, i) => (
  ...
  return (
    <div className="dlg-overlay" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="dlg">
        ...
        <textarea placeholder="Leave a summary comment (optional)…" value={body} onChange={e => setBody(e.target.value)} />
...
function Review({ tweaks, pr, clusterBots, updatedHunks }) {
  ...
  const addComment = (hunk, lineN, text, isReply) => {
  ...
  React.useEffect(() => {
    const onScroll = () => {
      const els = PR.chapters.map(c => ({ id: c.id, el: document.querySelector(`[data-chid='${c.id}']`) })).filter(x => x.el);
  ...
  return (
    <div className={reviewClasses}>
      <div className="prhead">
        <div className="row1">
          <h1><span className="num">#{PR.number}</span>{PR.title}</h1>
          <div className="actions">
            ...
            <button className="btn surface" onClick={() => setSubmitOpen(true)}>
              <Icon.Comment />Submit review
            </button>
          </div>
        </div>
        <div className="meta">
          <span className="branch"><b>{PR.branch}</b> → {PR.base}</span>
          <span><b style={{ color: "var(--fg-1)", fontWeight: 500 }}>{PR.author.name}</b> opened {PR.createdAt} · updated {PR.updatedAt}</span>
          <span>·</span>
          <span><span className="stat-add">+{PR.stats.additions}</span> <span className="stat-rem">−{PR.stats.deletions}</span> across {PR.stats.files} files</span>
          <span>·</span>
      ...
      <div className="submitbar">
        <div>
          <div className="progress"><b>{reviewedCount} of {totalCount}</b> chapters reviewed · <b>{pendingDrafts}</b> pending drafts</div>
          <div className="pgbar"><div className="fill" style={{ width: `${pct}%` }} /></div>
        </div>
        <button className="btn primary" onClick={() => setSubmitOpen(true)}>Submit review</button>
      </div>
      ...
      {submittedToast && (
        <div style={{
          ...
          Review submitted to GitHub · {submittedToast === "approve" ? "Approved ✓" : submittedToast === "request" ? "Changes requested" : "Comments posted"}
        </div>
      )}
    </div>
  );
