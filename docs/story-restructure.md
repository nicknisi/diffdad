...
- **R2 — "Overview digest rows navigate only; the inline strip acts."** Partially rejected. The resolve _toggle_ stays on digest rows: it is the same store action mutating the same state — one semantic channel with two access points, not the two-channels-two-semantics anti-pattern the diagnosis attacks — and forcing a scroll round-trip per note-severity finding would gut the triage loop the digest exists for. Comment/ask/lens are dropped from digest rows as the critique demands.
