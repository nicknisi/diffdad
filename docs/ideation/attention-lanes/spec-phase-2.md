...
| File Path                                           | Changes                                                                                                                                                                                                                                |
| --------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `packages/web/src/state/types.ts`                   | Add `triage?`, `dismissedAtSha?`, `reviewRollup?` to the web `Unit` mirror, matching the CLI shape. Add the `Lane` union.                                                                                                              |
...
- **Read the lane, don't compute it.** Partition on the unit's server-assigned lane. Where the field is absent (a unit that predates phase 1's backfill), fall back to today's `groupOf(status)` so the queue degrades to current behavior rather than to an empty screen.
